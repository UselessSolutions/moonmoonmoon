...
public class ContainerStarBackpack extends MenuAbstract {
	...
	@Override
	public IntList getMoveSlots(final @NotNull InventoryAction inventoryAction, final Slot slot, final int i, final Player entityPlayer) {
	...
	@Override
	public IntList getTargetSlots(final @NotNull InventoryAction inventoryAction, final Slot slot, final int i, final Player entityPlayer) {
		final int chestSize = this.backpackInventory.getContainerSize();
		return slot.index < chestSize ? this.getSlots(chestSize, 36, true) : this.getSlots(0, chestSize, false);
	}

	@Override
	public boolean stillValid(final @NotNull Player entityPlayer) {
